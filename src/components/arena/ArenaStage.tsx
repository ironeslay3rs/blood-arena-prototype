...
function FighterSprite({ fighter }: { fighter: FighterState }) {
  ...
  return (
    <div
      className="absolute bottom-10 flex w-20 -translate-x-1/2 flex-col items-center gap-1"
      style={{ left: `${leftPct}%` }}
    >
      <div
        className={`flex h-24 w-16 items-center justify-center rounded-md border-2 text-center text-[10px] font-bold leading-tight text-white shadow-md ${ring} ${
          fighter.role === "player"
...
export function ArenaStage({
  ...
  return (
    ...
    >
      ...
      <FighterSprite fighter={fighters[1]} />
    </section>
  );
}
